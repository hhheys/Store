...
async def add_to_cart(user_id, product_id, count):
    try:
        await db.execute("INSERT INTO carts (product_id, client_id, count) VALUES ($1, $2, $3)", product_id, user_id, count)
        return True
...
async def check_product_in_cart(product_id, user_id):
    res = await db.fetch("SELECT * FROM carts WHERE product_id = $1 AND client_id = $2", product_id, user_id)
    if len(res) == 0:
        return False
    return True

async def del_from_cart(user_id: int, product_id: int):
    await db.execute("DELETE FROM carts WHERE product_id = $1 AND client_id = $2", product_id, user_id)

async def del_all_cart(user_id: int):
    await db.execute("DELETE FROM carts WHERE client_id = $1", user_id)

async def get_all_cart(user_id) -> List[CartPosition]:
    res = await db.fetch("SELECT * FROM carts WHERE client_id = $1 LIMIT 10;", user_id)
    data = []
    for rec in res:
